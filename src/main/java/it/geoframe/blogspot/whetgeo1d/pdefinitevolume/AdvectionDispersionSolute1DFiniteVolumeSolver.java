...
@Description("This code solve the advection-dispersion solute equation written in conservative form."
...
@Keywords("Advection-dispersion solute equation, numerical solver, finite volume ")
@Bibliography("")
//@Label()
//@Name()
//@Status()
...
public class AdvectionDispersionSolute1DFiniteVolumeSolver {
	...
	public AdvectionDispersionSolute1DFiniteVolumeSolver( BoundaryCondition topBoundaryCondition, BoundaryCondition bottomBoundaryCondition, int KMAX) {
	...
	public double[] solve(double timeDelta, double bottomBCValue, double topBCValue, int KMAX, double[] kappasInterface, double[] conservedQuantitiesNew, double[] conservedQuantities, double[] spaceDeltaZ,
		...
		*/
		for(int i = 1; i < KMAX-1; i++) {
				...
				kP = kappasInterface[i+1] * conservedQuantities[i];
				kM = kappasInterface[i] * conservedQuantities[i];
		...
		// i == 0			
		kP = kappasInterface[1] * conservedQuantities[1];
		kM = kappasInterface[0] * conservedQuantities[0];
		...
		// i == KMAX -1
		kP = kappasInterface[KMAX] * conservedQuantities[KMAX];
		kM = kappasInterface[KMAX-1] * conservedQuantities[KMAX-1];
