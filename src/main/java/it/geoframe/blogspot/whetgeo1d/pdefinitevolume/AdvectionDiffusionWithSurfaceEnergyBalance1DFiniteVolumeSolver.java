...
//		this.topBoundaryCondition = topBoundaryCondition;		
	 ...
	 * @param timeDelta
	 */
	public double[] solve(double timeDelta, double bottomBCValue, double shortWaveIn, double longWaveIn, double surfaceAlbedo, double surfaceEmissivity, 
			double linearizedLongWaveOut, double sensibleHeatCoefficient, double airT, double rainT, int KMAX, double[] kappasInterface, 
		...
		*/
		...
		lowerDiagonal[KMAX-1] = -kM*timeDelta/spaceDeltaZ[KMAX-1] + timeDelta*transportedQuantities[KMAX-1]*( -0.5*velocities[KMAX-1] - 0.5*Math.abs(velocities[KMAX-1])); 
		
		mainDiagonal[KMAX-1] = conservedQuantitiesNew[KMAX-1] + kM*timeDelta/spaceDeltaZ[KMAX-1]  + timeDelta*linearizedLongWaveOut + timeDelta*sensibleHeatCoefficient + timeDelta*transportedQuantities[KMAX-1]*( 0.5*velocities[KMAX] + 0.5*Math.abs(velocities[KMAX]) - 0.5*velocities[KMAX-1] + 0.5*Math.abs(velocities[KMAX-1]) );
		
		upperDiagonal[KMAX-1] = 0.0;
		
		rhss[KMAX -1] = conservedQuantities[KMAX-1]*x[KMAX-1] +  timeDelta*((1-surfaceAlbedo)*shortWaveIn + surfaceEmissivity*longWaveIn + sensibleHeatCoefficient*airT - linearizedLongWaveOut*273.15)
				- timeDelta*transportedQuantities[KMAX-1]*( 0.5*velocities[KMAX]*rainT - 0.5*Math.abs(velocities[KMAX])*rainT )
