...
import org.geoframe.closureequation.closureequation.SoilWaterRetentionCurveFactory;
import org.geoframe.closureequation.conductivitymodel.ConductivityEquation;
import org.geoframe.closureequation.conductivitymodel.ConductivityEquationFactory;
...
public class ComputeQuantitiesSoluteAdvectionDispersion {
	 ...
	 * ) private InterfaceConductivity interfaceConductivity; private
...
//	private double tmp;
	...
	public void computeWaterVolumeConcentrations(int KMAX) {
		...
		for (int element = 0; element < KMAX; element++) {
			...
			variables.waterVolumeConcentrations[element] = variables.volumes[element]
					* variables.concentrations[element];
	...
	public void computeSoluteSourcesSinksTerm(int KMAX) {
		...
		for (int element = 0; element < KMAX; element++) {
			...
			;
			variables.sumSoluteSourceSinkTerm = variables.sumSoluteSourceSinkTerm
					+ variables.soluteSourcesSinksTerm[element];
		}
	}
	...
	public void computeWaterVolumeConcentrationsNew(int KMAX) {
		...
		for (int element = 0; element < KMAX; element++) {
			...
			variables.waterVolumeConcentrationsNew[element] = variables.volumesNew[element]
					* variables.concentrations[element];
			;
	...
	public void computeThetasInterface(int KMAX) {
		...
		for (int k = 1; k <= KMAX - 1; k++) {
			variables.thetasInterface[k] = interfaceDispersion.compute(variables.thetas[k - 1], variables.thetas[k],
	...
	public void computeTortuosityFactorsInterface(int KMAX) {
		...
		for (int element = 0; element <= KMAX - 1; element++) {
			...
			variables.tortuosityFactors[element] = Math.pow(variables.thetas[element], 2.33333333333333)
					/ Math.pow(parameters.thetaS[variables.parameterID[element]], 2);
		}

		// Computing tortuosity factor at the interface.
		for (int k = 1; k <= KMAX - 1; k++) {
			variables.tortuosityFactorsInterface[k] = interfaceDispersion.compute(variables.tortuosityFactors[k - 1],
					variables.tortuosityFactors[k], geometry.controlVolume[k - 1], geometry.controlVolume[k]);
		}
	...
	public void computeDispersionCoefficients(int KMAX) {
		...
		for (int element = 1; element <= KMAX - 1; element++) {
			...
			// CALCOLO DEL COEFFICIENTE DI DISPERSIONE secondo Bear(1972) da Stumpp et all.,
			// (2012)
			// The computation of the Dispersion Coefficient is in the interface of the
			// control volume.

			variables.dispersionCoefficients[element] = (parameters.longitudinalDispersivity[variables.parameterID[element]]
					* Math.abs(variables.darcyVelocities[element])) / variables.thetasInterface[element]
					+ parameters.molecularDiffusion[variables.parameterID[element]]
							* variables.tortuosityFactorsInterface[element];
			if (variables.thetasInterface[element] == 0) {
				variables.dispersionCoefficients[element] = 0;
			}
			if (Double.isNaN(variables.dispersionCoefficients[element])) {
				variables.dispersionCoefficients[element] = 0;
			}
			// System.out.println("variables.dispersionCoefficients is = "+
			// variables.dispersionCoefficients[element]);
		}
	...
	public void computeDispersionFactors(int KMAX) {
		...
		for (int element = 0; element <= KMAX; element++) {
			...
			variables.dispersionFactors[element] = variables.dispersionCoefficients[element]
					* variables.thetasInterface[element];
		}
	...
	public void computeDispersionSoluteFluxes(int KMAX) {
		...
		for (int k = 1; k <= KMAX - 1; k++) {
			variables.dispersionSoluteFluxes[k] = -variables.dispersionFactors[k]
					* (variables.concentrations[k] - variables.concentrations[k - 1]) / geometry.spaceDeltaZ[k + 1];
		}

		// bottom interface
		if (this.bottomBCType.equalsIgnoreCase("Bottom Dirichlet")
				|| this.bottomBCType.equalsIgnoreCase("BottomDirichlet")) {
			variables.dispersionSoluteFluxes[0] = -variables.dispersionFactors[0]
					* (variables.concentrations[0] - variables.soluteBottomBCValue) / geometry.spaceDeltaZ[0];
		} else {
		...
		if (this.topBCType.equalsIgnoreCase("Top Dirichlet") || this.topBCType.equalsIgnoreCase("TopDirichlet")) {
			variables.dispersionSoluteFluxes[KMAX] = -variables.dispersionFactors[KMAX]
					* (variables.soluteTopBCValue - variables.concentrations[KMAX - 1]) / geometry.spaceDeltaZ[KMAX];
		} else {
	...
	public void computeAdvectionSoluteFluxes(int KMAX) {
		...
		for (int k = 1; k <= KMAX - 1; k++) {
			variables.advectionSoluteFluxes[k] = 0.5 * variables.darcyVelocities[k]
					* (variables.concentrations[k] + variables.concentrations[k - 1])
					- 0.5 * Math.abs(variables.darcyVelocities[k])
							* (variables.concentrations[k] - variables.concentrations[k - 1]);
		}

		// bottom interface
		variables.advectionSoluteFluxes[0] = 0.5 * variables.darcyVelocities[0]
				* (variables.concentrations[0] + variables.soluteBottomBCValue)
		...
		// top interface
		variables.advectionSoluteFluxes[KMAX] = 0.5 * variables.darcyVelocities[KMAX]
				* (variables.soluteTopBCValue + variables.concentrations[KMAX - 1])
	 ...
	 * variables.errorInternalEnergy = variables.internalEnergyNew -
	 * variables.internalEnergy -
	 * timeDelta*(-variables.conductionHeatFluxs[KMAX]-variables.advectionHeatFluxs[
	 * KMAX] + variables.conductionHeatFluxs[0] + variables.advectionHeatFluxs[0]);
	 * 
	 * }
	 */

	public void computeError(int KMAX, double timeDelta) {
		variables.errorWaterVolumeConcentration = variables.waterVolumeConcentrationNew
				- variables.waterVolumeConcentration
				- timeDelta * (-variables.dispersionSoluteFluxes[KMAX] - variables.advectionSoluteFluxes[KMAX]
						+ variables.dispersionSoluteFluxes[0] + variables.advectionSoluteFluxes[0])
				+ variables.sumSoluteSourceSinkTerm;
		// variables.errorWaterVolumeConcentration =
		// variables.waterVolumeConcentrationNew - variables.waterVolumeConcentration -
		// timeDelta*(-variables.dispersionSoluteFluxes[KMAX] +
		// variables.dispersionSoluteFluxes[0]);
	}

	public void computeTimeVariationWaterVolumesConcentration(int KMAX, double timeDelta) {

		for (int k = 0; k <= KMAX - 1; k++) {
			variables.timeVariationWaterVolumesConcentration[k] = -timeDelta
					* (variables.dispersionSoluteFluxes[k + 1] + variables.advectionSoluteFluxes[k + 1]
							- variables.dispersionSoluteFluxes[k] - variables.advectionSoluteFluxes[k])
					+ variables.soluteSourcesSinksTerm[k];
			// variables.waterVolumesConcentrationNew - variables.waterVolumesConcentration
			// =
			// -timeDelta*(variables.dispersionSoluteFluxes[k+1]+variables.advectionSoluteFluxes[k+1]
			// + variables.dispersionSoluteFluxes[k] +
			// variables.advectionSoluteFluxes[k])+variables.soluteSourcesSinksTerm[k];
		}
