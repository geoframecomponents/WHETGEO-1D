...
public class ComputeQuantitiesRichardsRoot {
	...
	/*
	 * FIXME: check the extreme of the for loops in order to deal with Neumann BC
	 * and Dirichlet BC.
	 */
	...
	public void checkEvapoTranspirations(int KMAX) {
		...
		for (int element = 0; element < KMAX - 1; element++) {
			...
			if (variables.volumes[element] > thetaWP[variables.parameterID[element]]
					* geometry.controlVolume[element]) {
				if (variables.ETs[element] > (variables.volumes[element]
						- thetaWP[variables.parameterID[element]] * geometry.controlVolume[element])) {
					variables.ETs[element] = variables.volumes[element]
							- thetaWP[variables.parameterID[element]] * geometry.controlVolume[element];
					/*
					 * FIXME: add a warning if this happens?
					 */
					System.out.println(
							"\tETs volume larger than water volume available for evapotranspiration, ETs reduced.");
				}
			} else {
