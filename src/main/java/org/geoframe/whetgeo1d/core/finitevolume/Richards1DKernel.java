...
import org.geoframe.numerical.newtonalgorithm.NestedNewtonThomas;
...
public class Richards1DKernel {
	...
	public Richards1DKernel(IBoundaryCondition topBoundaryCondition,
			IBoundaryCondition bottomBoundaryCondition, int KMAX, int nestedNewton, double newtonTolerance,
			double delta, int MAXITER_NEWT, List<EquationState> equationState) {

		this.topBoundaryCondition = topBoundaryCondition;
		this.bottomBoundaryCondition = bottomBoundaryCondition;

		nestedNewtonAlg = new NestedNewtonThomas(nestedNewton, newtonTolerance, MAXITER_NEWT, KMAX, equationState,
				delta);
	 ...
	 * @param timeDelta
	 */
	public double[] solve(double timeDelta, double richardsBottomBCValue, double richardsTopBCValue, int KMAX,
			double[] kappasInterface, double[] volumes, double[] spaceDeltaZ, double[] ets, double[] waterSuctions,
			double[] temperatures, int[] parameterID, int[] rheologyID) {
		...
		lowerDiagonal[0] = bottomBoundaryCondition.lowerDiagonal(-999.0, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0],
				timeDelta);
		mainDiagonal[0] = bottomBoundaryCondition.mainDiagonal(-999.0, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0],
				timeDelta);
		upperDiagonal[0] = bottomBoundaryCondition.upperDiagonal(-999.0, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0],
				timeDelta);
		rhss[0] = volumes[0] + bottomBoundaryCondition.rightHandSide(richardsBottomBCValue, kP, kM, spaceDeltaZ[1],
				spaceDeltaZ[0], timeDelta) - ets[0];

		// i == KMAX -1
		kP = kappasInterface[KMAX];
		kM = kappasInterface[KMAX - 1];
		lowerDiagonal[KMAX - 1] = topBoundaryCondition.lowerDiagonal(-999.0, kP, kM, spaceDeltaZ[KMAX],
				spaceDeltaZ[KMAX - 1], timeDelta);
		mainDiagonal[KMAX - 1] = topBoundaryCondition.mainDiagonal(-999.0, kP, kM, spaceDeltaZ[KMAX],
				spaceDeltaZ[KMAX - 1], timeDelta);
		upperDiagonal[KMAX - 1] = topBoundaryCondition.upperDiagonal(-999.0, kP, kM, spaceDeltaZ[KMAX],
				spaceDeltaZ[KMAX - 1], timeDelta);
		rhss[KMAX - 1] = volumes[KMAX - 1] + topBoundaryCondition.rightHandSide(richardsTopBCValue, kP, kM,
				spaceDeltaZ[KMAX], spaceDeltaZ[KMAX - 1], timeDelta) - ets[KMAX - 1];
		...
		nestedNewtonAlg.set(waterSuctions, temperatures, mainDiagonal, upperDiagonal, lowerDiagonal, rhss, KMAX,
				parameterID, rheologyID);
