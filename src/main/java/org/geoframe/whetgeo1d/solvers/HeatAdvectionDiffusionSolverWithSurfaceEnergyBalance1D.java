...
import org.geoframe.whetgeo1d.core.derivedquantities.ComputeQuantitiesHeatAdvectionDiffusionWithSurfaceEnergyBalance;
...
public class HeatAdvectionDiffusionSolverWithSurfaceEnergyBalance1D extends HMModel {
	...
	// @Description("The HashMap with the time series of potential
	// evapotranspiration")
	// @In
	// @Unit ("W m-2")
	...
	private AdvectionDiffusionWithSurfaceEnergyBalance1DKernel advectionDiffusionSolver; // creare un nuovo
																										// solver con il
																										// surface
																										// energy
																										// balance
	...
	private ComputeQuantitiesHeatAdvectionDiffusionWithSurfaceEnergyBalance computeQuantitiesHeatAdvectionDiffusion; // creare
																														// un
																														// nuovo
																														// compute
																														// quantites
	...
	public void solve() {
		...
		if (step == 0) {
			...
			computeQuantitiesHeatAdvectionDiffusion = new ComputeQuantitiesHeatAdvectionDiffusionWithSurfaceEnergyBalance(
					typeClosureEquation, typeInternalEnergyEquationState, typeThermalConductivity,
					interfaceThermalConductivityModel, bottomInternalEnergyBCType, surfaceAlbedoType,
					surfaceEmissivityType, surfaceAereodynamicResistanceType, variables, geometry, parameters);
...
//			stressedETs = new double[KMAX];
			...
			bottomInternalEnergyBoundaryCondition = IBoundaryCondition.createDiffusionBoundaryCondition(
					DiffusionBoundaryConditionType.fromString(bottomInternalEnergyBCType));

			advectionDiffusionSolver = new AdvectionDiffusionWithSurfaceEnergyBalance1DKernel(
		...
		// if(topInternalEnergyBCType.equalsIgnoreCase("Top Neumann") ||
		// topInternalEnergyBCType.equalsIgnoreCase("TopNeumann")) {
		// variables.internalEnergyTopBCValue =
		// inInternalEnergyTopBC.get(stationID)[0]/tTimeStep;
		// } else {
		// variables.internalEnergyTopBCValue =
		// inInternalEnergyTopBC.get(stationID)[0]+273.15;
		// }

		variables.internalEnergyBottomBCValue = 0.0;
		if (bottomInternalEnergyBCType.equalsIgnoreCase("Bottom Neumann")
				|| bottomInternalEnergyBCType.equalsIgnoreCase("BottomNeumann")) {
		...
		while (sumTimeDelta < tTimeStep) {
			...
			computeQuantitiesHeatAdvectionDiffusion.computeLinearizedLongWaveOut(KMAX); // eventualmente da spostare
																						// sotto per considerare la lama
																						// d'acqua
...
//			System.out.println(variables.sensibleHeatCoefficient);
			...
			variables.temperatures = advectionDiffusionSolver.solve(timeDelta, variables.internalEnergyBottomBCValue,
					variables.shortWaveIn, variables.longWaveIn, surfaceAlbedo, surfaceEmissivity,
					variables.linearizedLongWaveOut, variables.sensibleHeatCoefficient, variables.airT, variables.rainT,
					KMAX, variables.lambdasInterface, variables.heatCapacitysNew, variables.heatCapacitys,
					geometry.spaceDeltaZ, variables.heatSourcesSinksTerm, variables.temperatures,
					variables.waterSuctions, variables.darcyVelocities, variables.waterCapacityTransported,
					variables.parameterID, variables.equationStateID);
