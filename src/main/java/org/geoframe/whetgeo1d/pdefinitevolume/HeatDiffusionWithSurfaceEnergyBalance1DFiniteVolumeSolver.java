...
public class HeatDiffusionWithSurfaceEnergyBalance1DFiniteVolumeSolver {
	...
	public HeatDiffusionWithSurfaceEnergyBalance1DFiniteVolumeSolver(IBoundaryCondition bottomBoundaryCondition,
			int KMAX, int nestedNewton, double newtonTolerance, double delta, int MAXITER_NEWT,
	...
	public double[] solve(double timeDelta, double bottomBCValue, double shortWaveIn, double longWaveIn,
			...
			double sensibleHeatCoefficient, double actualLatentHeatFlux, double airT, int KMAX,
			double[] kappasInterface, double[] conservedQuantities, double[] spaceDeltaZ, double[] sourceSinkTerm,
			double[] x, double[] y, int[] parameterID, int[] rheologyID) {
		...
		upperDiagonal[0] = bottomBoundaryCondition.upperDiagonal(-999.0, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0],
				timeDelta);
		rhss[0] = conservedQuantities[0] + bottomBoundaryCondition.rightHandSide(bottomBCValue, kP, kM, spaceDeltaZ[1],
				spaceDeltaZ[0], timeDelta) - sourceSinkTerm[0];

		// i == KMAX -1
		kP = kappasInterface[KMAX];
		kM = kappasInterface[KMAX - 1];
		lowerDiagonal[KMAX - 1] = -kM * timeDelta / spaceDeltaZ[KMAX - 1];
		mainDiagonal[KMAX - 1] = kM * timeDelta / spaceDeltaZ[KMAX - 1] + timeDelta * linearizedLongWaveOut
				+ timeDelta * sensibleHeatCoefficient;
		upperDiagonal[KMAX - 1] = 0.0;
		rhss[KMAX - 1] = conservedQuantities[KMAX - 1] + timeDelta * ((1 - surfaceAlbedo) * shortWaveIn
				+ surfaceEmissivity * longWaveIn + sensibleHeatCoefficient * airT + actualLatentHeatFlux)
