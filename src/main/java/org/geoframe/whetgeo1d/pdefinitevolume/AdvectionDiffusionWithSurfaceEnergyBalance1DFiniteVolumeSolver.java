...
package org.geoframe.whetgeo1d.pdefinitevolume;
...
public class AdvectionDiffusionWithSurfaceEnergyBalance1DFiniteVolumeSolver {
...
//	private BoundaryCondition topBoundaryCondition;
	...
	public AdvectionDiffusionWithSurfaceEnergyBalance1DFiniteVolumeSolver(IBoundaryCondition bottomBoundaryCondition,
			int KMAX) {
...
//		this.topBoundaryCondition = topBoundaryCondition;		
	 ...
	 * @param timeDelta
	 */
	public double[] solve(double timeDelta, double bottomBCValue, double shortWaveIn, double longWaveIn,
			double surfaceAlbedo, double surfaceEmissivity, double linearizedLongWaveOut,
			double sensibleHeatCoefficient, double airT, double rainT, int KMAX, double[] kappasInterface,
			double[] conservedQuantitiesNew, double[] conservedQuantities, double[] spaceDeltaZ,
			double[] sourceSinkTerm, double[] x, double[] y, double[] velocities, double[] transportedQuantities,
			int[] parameterID, int[] rheologyID) {

		this.timeDelta = timeDelta;
		this.KMAX = KMAX;

		/*
		 * COEFFICIENT MATRIX IS BUILD BY THREE VECTORS COLLECTING ELEMENTS OF THE THREE
		 * DIAGONAL: - lower diagonal x_(i+1) - main diagonal x_i - upper diagonal
		 * x_(i-1) - right-hand-side
		 */
		for (int i = 1; i < KMAX - 1; i++) {
			...
			lowerDiagonal[i] = -kM * timeDelta / spaceDeltaZ[i]
					+ timeDelta * transportedQuantities[i] * (-0.5 * velocities[i] - 0.5 * Math.abs(velocities[i]));

			mainDiagonal[i] = conservedQuantitiesNew[i] + kM * timeDelta / spaceDeltaZ[i]
					+ kP * timeDelta / spaceDeltaZ[i + 1]
					+ timeDelta * transportedQuantities[i] * (0.5 * velocities[i + 1]
							+ 0.5 * Math.abs(velocities[i + 1]) - 0.5 * velocities[i] + 0.5 * Math.abs(velocities[i]));

			upperDiagonal[i] = -kP * timeDelta / spaceDeltaZ[i + 1] + timeDelta * transportedQuantities[i]
					* (0.5 * velocities[i + 1] - 0.5 * Math.abs(velocities[i + 1]));
		...
		lowerDiagonal[0] = bottomBoundaryCondition.lowerDiagonal(-999.0, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0],
				timeDelta);

		mainDiagonal[0] = conservedQuantitiesNew[0]
				+ bottomBoundaryCondition.mainDiagonal(-999.0, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0], timeDelta)
				+ timeDelta * transportedQuantities[0] * (0.5 * velocities[1] + 0.5 * Math.abs(velocities[1])
						- 0.5 * velocities[0] + 0.5 * Math.abs(velocities[0]));

		upperDiagonal[0] = bottomBoundaryCondition.upperDiagonal(-999.0, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0],
				timeDelta)
				+ timeDelta * transportedQuantities[0] * (0.5 * velocities[1] - 0.5 * Math.abs(velocities[1]));

		rhss[0] = conservedQuantities[0] * x[0]
				+ bottomBoundaryCondition.rightHandSide(bottomBCValue, kP, kM, spaceDeltaZ[1], spaceDeltaZ[0],
						timeDelta)
				- timeDelta * transportedQuantities[0]
						* (-0.5 * velocities[0] * bottomBCValue - 0.5 * Math.abs(velocities[0]) * bottomBCValue)
				- sourceSinkTerm[0];

		// i == KMAX -1
		kP = kappasInterface[KMAX];
		kM = kappasInterface[KMAX - 1];
		lowerDiagonal[KMAX - 1] = -kM * timeDelta / spaceDeltaZ[KMAX - 1] + timeDelta * transportedQuantities[KMAX - 1]
				* (-0.5 * velocities[KMAX - 1] - 0.5 * Math.abs(velocities[KMAX - 1]));

		mainDiagonal[KMAX - 1] = conservedQuantitiesNew[KMAX - 1] + kM * timeDelta / spaceDeltaZ[KMAX - 1]
				+ timeDelta * linearizedLongWaveOut + timeDelta * sensibleHeatCoefficient
				+ timeDelta * transportedQuantities[KMAX - 1]
						* (0.5 * velocities[KMAX] + 0.5 * Math.abs(velocities[KMAX]) - 0.5 * velocities[KMAX - 1]
								+ 0.5 * Math.abs(velocities[KMAX - 1]));

		upperDiagonal[KMAX - 1] = 0.0;

		rhss[KMAX - 1] = conservedQuantities[KMAX - 1] * x[KMAX - 1]
				+ timeDelta * ((1 - surfaceAlbedo) * shortWaveIn + surfaceEmissivity * longWaveIn
						+ sensibleHeatCoefficient * airT - linearizedLongWaveOut * 273.15)
				- timeDelta * transportedQuantities[KMAX - 1]
						* (0.5 * velocities[KMAX] * rainT - 0.5 * Math.abs(velocities[KMAX]) * rainT)
