...
public class Whetgeo1DInputsHandler implements AutoCloseable {
	...
	/**
	 * Fixed name every timeseries table's single value column is written under,
	 * regardless of what the source CSV's own {@code @H} header called it
	 * (typically {@code value_<stationId>}) - normalizing at import time means
	 * the read side never has to discover or guess the column name.
	 */
	 ...
	 *
	 * <p>
	 * Reads the @H header line to find the value column (the CSV must carry
	 * exactly one, e.g. {@code timestamp,value_135}); the timestamp column is
	 * stored as Unix epoch millis (UTC), the value column as REAL - always
	 * under the fixed name {@link #COL_TIMESERIES_VALUE}, regardless of what
	 * the CSV header itself called it, so the read side never has to discover
	 * or guess the column name.
	 */
	private static void populateTemperature(File csv, String tableNameStr, ADb db) throws Exception {
		// Read @H header line to confirm there's exactly one value column
		...
		// headerParts: [@H, timestamp, value_<id>]
		final int numValueCols = headerParts.length - 2;
		if (numValueCols != 1) {
			throw new IllegalArgumentException(
					"Expected exactly one value column in " + csv.getName() + ", found " + numValueCols);
		}

		String[] fieldDefs = { COL_TEMPERATURE_TIMESTAMP + " INTEGER PRIMARY KEY", COL_TIMESERIES_VALUE + " REAL" };
		final String[] colNames = { COL_TEMPERATURE_TIMESTAMP, COL_TIMESERIES_VALUE };
